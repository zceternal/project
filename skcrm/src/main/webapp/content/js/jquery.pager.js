...
(function($) {
    ...
    $.fn.pager = function(options) {
        ...
        // empty out the destination element and then render out the pager with the supplied options
            $(this).empty().append(renderpager(parseInt(options.pagenumber),parseInt(options.pagesize), parseInt(options.pagecount),options.totalcount, options.buttonClickCallback,options.issel,options.selectChangeCallBack));
    ...
    function renderpager(pagenumber, pagesize, pagecount, totalcount, buttonClickCallback,issel,selectChangeCallBack) {
    	
        if (totalcount == undefined)
            totalcount = 0;
        // setup $pager to hold render
        var $pager = $('<ul class="pages" style="text-align: right;margin-right: 20px;"></ul>');
        $pager.append($('<span style="cursor:default">共 ' + totalcount + ' 条记录&nbsp;&nbsp;</span>'));
        ...
        if(issel)
        	{
		        var $select = ('&nbsp;&nbsp;&nbsp;每页<select onchange="'+selectChangeCallBack+'()" id="pageSizeNum"  style="margin: 0px 5px;height: 28px !important;">'
